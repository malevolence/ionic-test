(function(app) {
	...
	app.factory('authService', ['$http', '$q', 'localStorageService', 'baseUrl', function($http, $q, localStorageService, baseUrl) {
	...
	app.factory('JobRepo', ['$resource', 'baseUrl', function ($resource, baseUrl) {
		return $resource(baseUrl + '/jobs/:id', { jobId: '@id' });
	}]);

	app.factory('LookupRepo', ['$resource', 'baseUrl', function($resource, baseUrl) {
	...
	app.factory('ProfileRepo', ['$resource', 'baseUrl', function($resource, baseUrl) {
		return $resource(baseUrl + '/profiles/:id', { id: '@id' }, {
			profilegps: { method: 'GET', isArray: true, url: baseUrl + '/profiles/location' }
		});
	}]);

	app.factory('BlogRepo', ['$resource', 'baseUrl', function($resource, baseUrl) {
